...
#
# Usage: check_tropes.sh [git-pathspec ...]   (default: '*.md' '*.qmd')
# Runs from the repo root; scans tracked files via `git ls-files`. Exit 1 on a hit.
set -uo pipefail
...
[ "${#files[@]}" -eq 0 ] && { echo "tropes check: no matching files"; exit 0; }
...
# scan LABEL CASE PATTERN...  (CASE = "i" for case-insensitive, else case-sensitive).
# Runs in the current shell so `found` survives.
scan() {
